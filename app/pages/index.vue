...
<template>
  <div>
    <div class="px-8 pt-8 pb-6 border-b border-border">
      ...
      <p class="mt-1 text-sm text-muted">{{ (stats as any)?.month }} · Live data</p>
    </div>
    ...
    <div class="px-8 py-8">
      ...
      <div class="grid grid-cols-4 gap-px bg-border mb-8">
        <div class="bg-bg px-6 py-5">
          ...
          <p class="text-xs text-muted mt-0.5">ETB incl. VAT</p>
          <p class="text-xs text-muted mt-2 pt-2 border-t border-border">
            {{ (stats as any)?.sales?.count ?? 0 }} invoices · VAT {{ fmt((stats as any)?.sales?.vat ?? 0) }} ETB
          </p>
        </div>
        <div class="bg-bg px-6 py-5">
          ...
          <p class="text-xs text-muted mt-0.5">ETB incl. VAT</p>
          <p class="text-xs text-muted mt-2 pt-2 border-t border-border">
            {{ (stats as any)?.purchases?.count ?? 0 }} purchase orders
          </p>
        </div>
        <div class="bg-bg px-6 py-5">
          ...
          <p class="text-xs text-muted mt-0.5">ETB</p>
          <p class="text-xs text-muted mt-2 pt-2 border-t border-border">Sales ex-VAT minus purchases</p>
        </div>
        <div class="bg-bg px-6 py-5">
          ...
          <p class="text-xs text-muted mt-0.5">items</p>
          <p class="text-xs text-muted mt-2 pt-2 border-t border-border">Stock qty below 5</p>
        </div>
      </div>

      <!-- Low stock alert -->
      <div v-if="(stats as any)?.lowStockItems?.length > 0">
        <h2 class="font-display text-base font-bold text-text mb-3">Low Stock Alert</h2>
        <div class="border border-border">
          ...
          >
            ...
            <span class="font-mono text-sm text-red font-medium">{{ item.stockQty }} {{ item.unit }} remaining</span>
          </div>
        </div>
      </div>
